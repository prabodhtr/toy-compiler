...
program 		: TypeDefBlock ClassDefBlock GDeclBlock FDefBlock MainBlock			{XsmHeader();ASTnode = createTree(0,CONNECT_NODE,NULL,-1,$4,NULL,$5,NULL);
																					 $$ = createTree(0,CONNECT_NODE,NULL,-1,$2,NULL,ASTnode,NULL);
																					 CodeGen($$,fout);}
				| TypeDefBlock ClassDefBlock GDeclBlock MainBlock					{XsmHeader();$$ = createTree(0,CONNECT_NODE,NULL,-1,$2,NULL,$4,NULL);CodeGen($$,fout);}
				| TypeDefBlock ClassDefBlock MainBlock								{XsmHeader();$$ = createTree(0,CONNECT_NODE,NULL,-1,$2,NULL,$3,NULL);CodeGen($$,fout);}
				;	
...
Cname         	: ID       									{ if(TLookup($1->varname) != NULL){
...
MDecl      		: Type ID '(' ParamList ')' SEMICOLON 		{
															 ...
															 if(Class_Mlookup(cur_class,$2->varname) != NULL){
																 printf("overloading of %s() is not allowed\n");
																 exit(1);
															 }
...
int main(void) {
	...
	fout = fopen("dataencap.xsm","w");
	fin = fopen("fact_sum.cod","r");
	yyin = fin;
